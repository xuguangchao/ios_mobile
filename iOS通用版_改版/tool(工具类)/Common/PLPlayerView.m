...
//        [self.player seekTo:CMTimeMake(6*1000, 1000)];
//    });
}
...
- (void)timerAction {
    ...
    if (CMTimeGetSeconds(self.player.totalDuration)) {
        ...
        NSString *current = [NSString stringWithFormat:@"%.f",CMTimeGetSeconds(self.player.currentTime)];
        NSString *total = [NSString stringWithFormat:@"%.f",CMTimeGetSeconds(self.player.totalDuration)];
